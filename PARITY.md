...
- Hook config can be inspected via Rust config reporting in `rust/crates/commands/src/lib.rs` and `rust/crates/fraude-cli/src/main.rs`.
...
## skills/ and CLAW.md / FRAUDE.md discovery
...
- Main CLI/repl/prompt handling lives in `rust/crates/fraude-cli/src/main.rs`.
...
- Stream/tool event translation in `rust/crates/fraude-cli/src/main.rs`.
...
- **Prompt mode tools enabled**
  - `rust/crates/fraude-cli/src/main.rs` now constructs prompt mode with `LiveCli::new(model, true, ...)`.
- **Default permission mode = DangerFullAccess**
  - Runtime default now resolves to `DangerFullAccess` in `rust/crates/fraude-cli/src/main.rs`.
  - Clap default also uses `DangerFullAccess` in `rust/crates/fraude-cli/src/args.rs`.
  - Init template writes `dontAsk` in `rust/crates/fraude-cli/src/init.rs`.
- **Streaming `{}` tool-input prefix bug**
  - `rust/crates/fraude-cli/src/main.rs` now strips the initial empty object only for streaming tool input, while preserving legitimate `{}` in non-stream responses.
