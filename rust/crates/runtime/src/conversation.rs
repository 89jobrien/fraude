    ...
    T: ToolExecutor,
{
    ...
    ) -> Result<Crux<TurnSummary>, RuntimeError> {
                ...
                let result_message = match permission_outcome {
                    PermissionOutcome::Allow => {
                        ...
                        if pre_hook_result.is_denied() {
                            ...
                            steps.push(Step {
                                ...
                                content_hash: Some(hash_str(&output_text)),
                                output: Some(serde_json::Value::String(output_text.clone())),
                                error: Some(output_text.clone()),
                                attempt: 1,
                                events: Vec::new(),
                            ...
                            steps.push(Step {
                                ...
                                duration_ms,
                                input_hash: hash_str(&input),
                                content_hash: Some(hash_str(&output)),
                                output: Some(serde_json::Value::String(output.clone())),
                                error: if is_error { Some(output.clone()) } else { None },
                                attempt: 1,
                                events: Vec::new(),
                                metadata: std::collections::HashMap::new(),
                                findings: Vec::new(),
                    ...
                    PermissionOutcome::Deny { reason } => {
                        ...
                        steps.push(Step {
                            ...
                            duration_ms,
                            input_hash: hash_str(&input),
                            content_hash: Some(hash_str(&reason)),
                            output: Some(serde_json::Value::String(reason.clone())),
