...
fn discover_instruction_files(cwd: &Path) -> std::io::Result<Vec<ContextFile>> {
    ...
    for dir in directories {
        for candidate in [
            dir.join("FRAUDE.md"),
            dir.join("FRAUDE.local.md"),
            dir.join(".fraude").join("FRAUDE.md"),
            dir.join(".fraude").join("instructions.md"),
        ] {
...
fn render_project_context(project_context: &ProjectContext) -> String {
    ...
    if !project_context.instruction_files.is_empty() {
        bullets.push(format!(
            "Fraude instruction files discovered: {}.",
...
fn render_instruction_files(files: &[ContextFile]) -> String {
    let mut sections = vec!["# Fraude instructions".to_string()];
...
fn render_config_section(config: &RuntimeConfig) -> String {
    ...
    if config.loaded_entries().is_empty() {
        lines.extend(prepend_bullets(vec![
            "No Fraude settings files loaded.".to_string(),
        ]));
        return lines.join("\n");
    }
...
mod tests {
    ...
    fn discovers_instruction_files_from_ancestor_chain() {
        ...
        fs::create_dir_all(nested.join(".fraude")).expect("nested fraude dir");
        fs::write(root.join("FRAUDE.md"), "root instructions").expect("write root instructions");
        fs::write(root.join("FRAUDE.local.md"), "local instructions")
            .expect("write local instructions");
        fs::create_dir_all(root.join("apps")).expect("apps dir");
        fs::create_dir_all(root.join("apps").join(".fraude")).expect("apps fraude dir");
        fs::write(root.join("apps").join("FRAUDE.md"), "apps instructions")
        ...
        fs::write(
            nested.join(".fraude").join("instructions.md"),
            "nested instructions",
        )
    ...
    fn dedupes_identical_instruction_content_across_scopes() {
        ...
        fs::write(root.join("FRAUDE.md"), "same rules\n\n").expect("write root");
        fs::write(nested.join("FRAUDE.md"), "same rules\n").expect("write nested");
    ...
    fn displays_context_paths_compactly() {
        assert_eq!(
            display_context_path(Path::new("/tmp/project/.fraude/FRAUDE.md")),
            "FRAUDE.md"
        );
    }
    ...
    fn discover_with_git_includes_status_snapshot() {
        ...
        fs::write(root.join("FRAUDE.md"), "rules").expect("write instructions");
        ...
        assert!(status.contains("?? FRAUDE.md"));
    ...
    fn load_system_prompt_reads_fraude_files_and_config() {
        let root = temp_dir();
        fs::create_dir_all(root.join(".fraude")).expect("fraude dir");
        fs::write(root.join("FRAUDE.md"), "Project rules").expect("write instructions");
        fs::write(
            root.join(".fraude").join("settings.json"),
...
",
    ...
    fn renders_fraude_style_sections_with_project_context() {
        let root = temp_dir();
        fs::create_dir_all(root.join(".fraude")).expect("fraude dir");
        fs::write(root.join("FRAUDE.md"), "Project rules").expect("write FRAUDE.md");
        fs::write(
            root.join(".fraude").join("settings.json"),
            r#"{"permissionMode":"acceptEdits"}"#,
        )
        ...
        assert!(prompt.contains("# Fraude instructions"));
        assert!(prompt.contains("Project rules"));
        assert!(prompt.contains("permissionMode"));
    ...
    fn discovers_dot_fraude_instructions_markdown() {
        ...
        assert!(
            context
                .instruction_files
                .iter()
                .any(|file| file.path.ends_with(".fraude/instructions.md"))
        );
        assert!(
    ...
    fn renders_instruction_file_metadata() {
        let rendered = render_instruction_files(&[ContextFile {
            path: PathBuf::from("/tmp/project/FRAUDE.md"),
            content: "Project rules".to_string(),
        }]);
        assert!(rendered.contains("# Fraude instructions"));
