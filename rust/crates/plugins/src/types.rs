...
/// Shared data for all plugin kinds. The behavioural differences (validation,
/// lifecycle) are encoded at the [`PluginDefinition`] enum level, not here.
#[derive(Debug, Clone, PartialEq)]
...
/// Type aliases preserve the public API surface for downstream crates that
/// already import `BuiltinPlugin`, `BundledPlugin`, or `ExternalPlugin`.
pub type BuiltinPlugin = PluginData;
pub type BundledPlugin = PluginData;
pub type ExternalPlugin = PluginData;
...
impl PluginDefinition {
    fn data(&self) -> &PluginData {
        match self {
            Self::Builtin(data) | Self::Bundled(data) | Self::External(data) => data,
        }
    }
}
...
impl Plugin for PluginDefinition {
    ...
    fn validate(&self) -> Result<(), PluginError> {
        match self {
            Self::Builtin(_) => Ok(()),
            Self::Bundled(data) | Self::External(data) => {
                validate_hook_paths(data.metadata.root.as_deref(), &data.hooks)?;
                validate_lifecycle_paths(data.metadata.root.as_deref(), &data.lifecycle)?;
                validate_tool_paths(data.metadata.root.as_deref(), &data.tools)
            }
        }
    }
    ...
    fn initialize(&self) -> Result<(), PluginError> {
        match self {
            Self::Builtin(_) => Ok(()),
            Self::Bundled(data) | Self::External(data) => run_lifecycle_commands(
                &data.metadata,
                &data.lifecycle,
                "init",
                &data.lifecycle.init,
...
pub fn builtin_plugins() -> Vec<PluginDefinition> {
    vec![PluginDefinition::Builtin(PluginData {
        metadata: PluginMetadata {
