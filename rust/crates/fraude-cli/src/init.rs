...
const STARTER_FRAUDE_JSON: &str = concat!(
    "{\n",
    "  \"permissions\": {\n",
    "    \"defaultMode\": \"dontAsk\"\n",
    "  }\n",
...
pub(crate) fn render_init_fraude_md(cwd: &Path) -> String {
    ...
    lines.push("- Keep shared defaults in `.fraude.json`; reserve `.fraude/settings.local.json` for machine-local overrides.".to_string());
    lines.push("- Do not overwrite existing `FRAUDE.md` content automatically; update it intentionally when repo workflows change.".to_string());
    lines.push(String::new());
...
#[cfg(test)]
mod tests {
    use super::{initialize_repo, render_init_fraude_md};
    use std::fs;
    use std::path::Path;
    ...
    fn temp_dir() -> std::path::PathBuf {
        let nanos = SystemTime::now()
            ...
            .as_nanos();
        std::env::temp_dir().join(format!("fraude-init-{nanos}"))
    }
    ...
    fn initialize_repo_creates_expected_files_and_gitignore_entries() {
        ...
        assert!(rendered.contains(".fraude/         created"));
        assert!(rendered.contains(".fraude.json     created"));
        assert!(rendered.contains(".gitignore       created"));
        assert!(rendered.contains("FRAUDE.md        created"));
        assert!(root.join(".fraude").is_dir());
    ...
    fn initialize_repo_is_idempotent_and_preserves_existing_files() {
        ...
        assert!(second_rendered.contains(".fraude.json     skipped (already exists)"));
        ...
        let gitignore = fs::read_to_string(root.join(".gitignore")).expect("read gitignore");
        assert_eq!(gitignore.matches(".fraude/settings.local.json").count(), 1);
        assert_eq!(gitignore.matches(".fraude/sessions/").count(), 1);
    ...
    fn render_init_template_mentions_detected_python_and_nextjs_markers() {
        ...
        let rendered = render_init_fraude_md(Path::new(&root));
