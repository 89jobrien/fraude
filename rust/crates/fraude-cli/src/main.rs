...
fn render_cli_error(problem: &str) -> String {
    ...
    lines.push("  Help             fraude --help".to_string());
    lines.join("\n")
}
...
fn parse_args(args: &[String]) -> Result<CliAction, String> {
    ...
    while index < args.len() {
        match args[index].as_str() {
            ...
            "-p" => {
                // Fraude compat: -p "prompt" = one-shot prompt
            ...
            "--print" => {
                // Fraude compat: --print makes output non-interactive
...
fn default_permission_mode() -> PermissionMode {
    env::var("FRAUDE_PERMISSION_MODE")
        .ok()
        .as_deref()
...
fn default_oauth_config() -> OAuthConfig {
    OAuthConfig {
        ...
        authorize_url: String::from("https://platform.fraude.dev/oauth/authorize"),
        token_url: String::from("https://platform.fraude.dev/v1/oauth/token"),
        callback_port: None,
...
fn run_login() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let client = FraudeApiClient::from_auth(AuthSource::None).with_base_url(api::read_base_url());
...
impl LiveCli {
    ...
    fn handle_unavailable_command(&self, command: &SlashCommand) -> bool {
        ...
        eprintln!("{}", render_mode_unavailable(name, description));
        false
    }
    ...
    fn run_internal_prompt_text_with_progress(
        ...
        let mut permission_prompter = CliPermissionPrompter::new(self.permission_mode);
        let summary = runtime
            .run_turn(prompt, Some(&mut permission_prompter))?
            .into_value()
            .map_err(|e| Box::<dyn std::error::Error>::from(e.to_string()))?;
    ...
    fn run_pr(&self, context: Option<&str>) -> Result<(), Box<dyn std::error::Error>> {
        ...
        if command_exists("gh") {
            let body_path = write_temp_text_file("fraude-pr-body.md", &body)?;
    ...
    fn run_issue(&self, context: Option<&str>) -> Result<(), Box<dyn std::error::Error>> {
        ...
        if command_exists("gh") {
            let body_path = write_temp_text_file("fraude-issue-body.md", &body)?;
            let output = Command::new("gh")
...
fn sessions_dir() -> Result<PathBuf, Box<dyn std::error::Error>> {
    let cwd = env::current_dir()?;
    let path = cwd.join(".fraude").join("sessions");
...
fn init_fraude_md() -> Result<String, Box<dyn std::error::Error>> {
    let cwd = env::current_dir()?;
    Ok(initialize_repo(&cwd)?.render())
}

fn run_init() -> Result<(), Box<dyn std::error::Error>> {
    println!("{}", init_fraude_md()?);
    Ok(())
}
...
fn write_examples_section(out: &mut impl Write) -> io::Result<()> {
...
fn print_help_to(out: &mut impl Write) -> io::Result<()> {
    ...
    write_commands_section(out)?;
    write_flags_section(out)?;
    writeln!(out, "Slash command reference")?;
    writeln!(out, "{}", render_slash_command_help())?;
    writeln!(out)?;
    write_examples_section(out)
}
...
mod tests {
    ...
    fn init_help_mentions_direct_subcommand() {
        ...
        assert!(help.contains("fraude init"));
        assert!(help.contains("fraude agents"));
        assert!(help.contains("fraude skills"));
        assert!(help.contains("fraude /skills"));
    }
 ...
 M src/main.rs",
    ...
    fn init_template_mentions_detected_rust_workspace() {
        let rendered = crate::init::render_init_fraude_md(std::path::Path::new("."));
        assert!(rendered.contains("# FRAUDE.md"));
    ...
    fn ultraplan_progress_lines_include_phase_step_and_elapsed_status() {
        let snapshot = InternalPromptProgressState {
            ...
            detail: Some("reading rust/crates/fraude-cli/src/main.rs".to_string()),
            saw_final_text: false,
        };
    ...
    fn describe_tool_progress_summarizes_known_tools() {
        ...
        assert!(
            describe_tool_progress("bash", r#"{"command":"cargo test -p fraude-cli"}"#)
                .contains("cargo test -p fraude-cli")
        );
        assert_eq!(
